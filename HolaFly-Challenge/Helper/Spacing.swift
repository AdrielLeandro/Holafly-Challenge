//
//  Spacing.swift
//  HolaFly-Challenge
//
//  Created by Adriel Pinzas on 25/04/24.
//

import Foundation

struct Spacing {
    public static let s2: CGFloat = 2.0
    public static let s4: CGFloat = 4.0
    public static let s6: CGFloat = 6.0
    public static let s8: CGFloat = 8.0
    public static let s10: CGFloat = 10.0
    public static let s12: CGFloat = 12.0
    public static let s16: CGFloat = 16.0
    public static let s20: CGFloat = 20.0
    public static let s24: CGFloat = 24.0
    public static let s26: CGFloat = 26.0
    public static let s28: CGFloat = 28.0
    public static let s30: CGFloat = 30.0
    public static let s32: CGFloat = 32.0
    public static let s50: CGFloat = 50.0
    public static let s84: CGFloat = 84.0
}
